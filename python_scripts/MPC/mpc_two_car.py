...
def main():
    try:
        ...
        env.obst_state = env.spawn_vehicle_stationary(135, waypoints)
        for i in range(50):
            env.world.tick()
        ...
        env.set_opti_weights(w_u0 = env.w_matrix(1, 0)[:-1, :-1], w_u1 = env.w_matrix(2, 0)[:-1, :-1], w_ql = env.w_matrix(3, -0.02), 
                            gamma = env.w_matrix(8, -0.2), w_c = 5, w_ds = env.w_matrix(init_value=1, step_size=1)[:-1, :-1], w_obs = env.w_matrix(init_value=8, step_size=0)[:-1, :-1])
        Ts = 0.1
