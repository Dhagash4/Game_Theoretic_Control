...
class CarEnv():
    ...
    def destroy(self) -> NoReturn:
        """Destroy all actors in the world and end synchronous mode for carla
        """
    ...
    def set_opti_weights(self, weights: dict) -> NoReturn:
        ...
        """
        self.w_gas = weights['w_gas']           # Throttle/brake penalty
        self.w_steer = weights['w_steer']       # Steer penalty
        self.w_lag = weights['w_lag']           # Lag Error penalty
        self.w_reward = weights['w_reward']     # Path progress reward
        self.w_roc = weights['w_roc']           # Rate of change of controls and velocity penalty 
        self.w_b = weights['w_b']               # Boundary constraints penalty
    ...
    def fit_curve(self, waypoints: np.ndarray, L: np.ndarray) -> Tuple[casadi.Function, casadi.Function, casadi.Function, casadi.Function]:
        ...
        - lut_boundary_cost: Look up table for the track boundary constraints
        """
        ...
        return self.lut_x, self.lut_y, self.lut_theta, self.lut_boundary_cost
    ...
    def mpc(self, prev_vals: dict, system_params: np.ndarray, max_L: float, L: np.ndarray, Ts: float, orient_flag: bool, plot_flag: bool, print_flag: bool) -> (dict):
        ...
        p_acc   =  u[0, :]  @ self.w_gas @ u[0, :].T                             # Throttle/Brake magnitude
        p_steer =  u[1, :] @  self.w_steer @ u[1, :].T                           # Steering magnitude
        p_lag = lag_error @ self.w_lag @ lag_error.T                             # MPCC Lag error
        p_boundary  = track_boundary @ self.w_b @ track_boundary.T               # Soft distance cost

        p_control_roc = self.w_roc * sumsqr(u[:, :self.P-1] - u[:, 1:self.P])    # Rate of Change of Controls
        p_v_roc = self.w_roc * sumsqr(v[:self.P] - v[1:self.P + 1])              # Rate of Change of path velocity 
        p_wobble = 1.5 * sumsqr(centerline_offset[1:] - centerline_offset[:-1])  # Wobble of path wrt centerline
        
        r_v_max = ((v * Ts) @ self.w_reward @ (v * Ts).T)                        # Path Progression Reward

        # Minimization objective
        opti.minimize(p_acc + p_steer + p_lag + p_boundary + p_control_roc + p_v_roc + p_wobble - r_v_max)
...
def main():
    try:
        ...
        argparser.add_argument('-f', '--filename', default='mpc_1_car', type=str, help='Filename for the data logs')
        ...
        argparser.add_argument('--vmax', default=35, type=float, help='Set maximum velocity for the vehicle')
        argparser.add_argument('--spawn-idx', default=0, type=int, help='Set the spawn index')
        args = argparser.parse_args()

        if args.save:
            states_filename = '../../Data/MPC/' + args.filename + '_states.pickle'
            controls_filename = '../../Data/MPC/' + args.filename + '_controls.pickle'
            states_file = open(states_filename, 'wb')
            controls_file = open(controls_filename, 'wb')


        # Spawn a vehicle at spawn_pose
        spawn_idx = args.spawn_idx
        env.nearest_wp_idx = args.spawn_idx
        vehicle, env.car_state, max_steer_angle = env.spawn_vehicle_2D(spawn_idx, waypoints, 0)

        if args.save:
            cpickle.dump(np.r_[env.car_state], states_file)
            cpickle.dump(np.r_[0.0, 0.0, 0.0], controls_file)
        ...
        env.lut_x, env.lut_y, env.lut_theta, env.lut_boundary_cost = env.fit_curve(waypoints_ext, L)
        ...
        weights = {}
        weights['w_gas'] = env.w_matrix(1, 0)[:-1, :-1]
        weights['w_steer'] = env.w_matrix(2, 0)[:-1, :-1]
        weights['w_lag'] = env.w_matrix(3, -0.02)
        weights['w_reward'] = env.w_matrix(8, -0.2)
        weights['w_b'] = env.w_matrix(init_value=1, step_size=0)[:-1, :-1]
        weights['w_roc'] = 5
        ...
        while(1):
            ...
            try:
                prev_vals = env.mpc(prev_vals, sys_params, max_L, L, Ts, orient_flag, args.plot, args.print)
            ...
            if args.save:
                cpickle.dump(env.car_state, states_file)
                cpickle.dump(np.r_[throttle, brake, steer], controls_file)
