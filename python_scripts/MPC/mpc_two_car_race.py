...
class CarEnv():
    ...
    def w_matrix(self,init_value,step_size,P):
        w = np.eye(P+1)
        for i in range(P+1):
    ...
    def dist_to_ellipse(self, ref_state, state_mpc, a, b):
        ...
        d = ((((x - x_c) * cos(theta) - (y - y_c) * sin(theta)) ** 2) / (a ** 2)) + ((((x - x_c) * sin(theta) + (y - y_c) * cos(theta)) ** 2) / (b ** 2))
    ...
    def mpc(self, orient_flag, prev_states, prev_controls, prev_t, prev_v, curr_state, nearest_idx, obs_state, system_params: np.ndarray, max_L: float, Ts: float,P: int,vmax: int,w_u0: float, w_u1: float, w_ql: float, gamma: float, w_c: float, w_ds: float):
        ...
        s = opti.variable(5, P + 1)    # States [x, y, yaw, vx, vy]
        e = opti.variable(1, P + 1)    # Lag Error
        t = opti.variable(1, P + 1)    # Path length parameter
        v = opti.variable(1, P + 1)    # Path progression rate
        u = opti.variable(2, P)        # Controls [throttle/brake, steering]
        d_s = opti.variable(1, P)
        c = opti.variable(1, P)
        ...
        p_lag = e @ w_ql @ e.T                                             # Lag error
        c_d_s  = d_s @ w_ds @ d_s.T                                        # Soft distance cost
        ...
        opti.subject_to(opti.bounded(0, v, vmax))          # Bounded path progression
        ...
        for i in range(P):
            ...
            d = self.dist_to_ellipse(obs_state, s[:, i + 1], 3, 3)
            opti.subject_to(d > 1)
        ...
        if orient_flag:
            opti.set_initial(s, np.vstack([curr_state] * (P + 1)).T)
        else:
    ...
    try:
        ...
        while(1):
            ...
            if not end_flag[0]:
                P1 = 25
                vmax1 = 20
                w_u01 = env.w_matrix(1, 0,P1)[:-1, :-1]
                w_u11 = env.w_matrix(2, 0,P1)[:-1, :-1]
                w_ql1 = env.w_matrix(2, -0.02,P1)
                gamma1 = env.w_matrix(8, -0.2,P1)
                w_c1 = 5
                w_ds1 = env.w_matrix(1, 1.5, P1)[:-1, :-1]
                try:
                    prev_states_1, prev_controls_1, prev_t_1, prev_v_1 = env.mpc(orient_flag_1, prev_states_1, prev_controls_1, prev_t_1, prev_v_1, env.car_1_state, env.nearest_idx_c1, env.car_2_state, system_params, waypoints.shape[0] + 1, Ts,P1,vmax1,w_u01,w_u11,w_ql1,gamma1,w_c1,w_ds1)
                    
                    steer_1 = prev_controls_1[1, 0] / 1.22

                    if prev_controls_1[0, 0] > 0:
                        throttle_1 = prev_controls_1[0, 0]
                        brake_1 = 0
                    else:
                    ...
                    prev_states_1 = np.vstack([env.car_1_state] * (env.P + 1)).T
                    prev_t_1 = prev_t_1 + env.car_1_state[3] * Ts
            ...
            if not end_flag[1]:
                P2 = 25
                vmax2 = 35
                w_u02 = env.w_matrix(1, 0,P2)[:-1, :-1]
                w_u12 = env.w_matrix(2, 0,P2)[:-1, :-1]
                w_ql2 = env.w_matrix(3, -0.02,P2)
                gamma2 = env.w_matrix(8, -0.2,P2)
                w_c2 = 5
                w_ds2 = env.w_matrix(1, 1.5, P1)[:-1, :-1]
                try:
                    prev_states_2, prev_controls_2, prev_t_2, prev_v_2 = env.mpc(orient_flag_2, prev_states_2, prev_controls_2, prev_t_2, prev_v_2, env.car_2_state, env.nearest_idx_c2, env.car_1_state, system_params, waypoints.shape[0] + 1, Ts,P2,vmax2,w_u02,w_u12,w_ql2,gamma2,w_c2,w_ds2)
                    
                    steer_2 = prev_controls_2[1, 0] / 1.22

                    if prev_controls_2[0, 0] > 0:
                        throttle_2 = prev_controls_2[0, 0]
                        brake_2 = 0
                    else:
        ...
        # Save all Dataclass object lists to a file
            env.save_log('../../Data/states_1_mpc.pickle', env.states_1)
            env.save_log('../../Data/controls_1_mpc.pickle', env.controls_1)
            env.save_log('../../Data/states_2_mpc.pickle', env.states_2)
            env.save_log('../../Data/controls_2_mpc.pickle', env.controls_2)
