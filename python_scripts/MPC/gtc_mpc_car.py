...
class CarEnv():
    ...
    def fit_curve(self, waypoints: np.ndarray, L: np.ndarray) -> Tuple[casadi.Function, casadi.Function, casadi.Function, casadi.Function, casadi.Function]:
        ...
        for i in range(100000):
            if numbers[i] >= 1:
                ...
                cost_fit[i] = ((1 - numbers[i]) * 1) 
        self.lut_collide = interpolant('LUT_collide', 'bspline', [numbers], cost_fit, dict(degree=[3]))

        return self.lut_x, self.lut_y, self.lut_theta, self.lut_d, self.lut_collide
    ...
    def dist_to_ellipse(self, ref_states: np.ndarray, state_mpc: casadi.MX, a: float, b: float) -> (casadi.MX):
        x_c = ref_states[0]
        y_c = ref_states[1]
        theta = -ref_states[2]
    ...
    def mpc(self, orient_flag: bool, prev_vals: dict, curr_state: np.ndarray, nearest_idx: int, obs_states: np.ndarray, system_params: np.ndarray, max_L: float, L: np.ndarray, Ts: float) -> (dict):
        ...
        c = opti.variable(1, self.P)                    # Change in lateral position of vehicle
        collision = opti.variable(1, self.P)            # bounding ellipse collision penalty
        ...
        collision_cost = collision @ self.w_col @ collision.T                   # Collision cost

        # Minimization objective
        opti.minimize(p_lag + p_control_roc + p_v_roc + p_acc + p_steer - r_v_max + 2.5 * sumsqr(c[1:] - c[:-1]) + c_d_s
                      + collision_cost)
        ...
        for i in range(self.P):
            ...
            opti.subject_to(collision[i] == self.lut_collide(d))
            # opti.subject_to(d > 1)
        ...
        # Set ipopt options
        p_opts = {"print_time": False, 'ipopt.print_level': 0, "ipopt.expect_infeasible_problem": "yes", "ipopt.max_iter": 50}
...
def main():
    try:
        ...
        num_of_laps = [1, 1]            # Desired number of laps
        ...
        vehicle_2, env.car_2_state, max_steer_angle_2 = env.spawn_vehicle_2D(spawn_idx+4, waypoints,  4)
        ...
        obs_state_1 = prev_vals_2['states']
        obs_state_2 = prev_vals_1['states']
        ...
        while(1):
            ...
            for i in range(iters):
                ...
                if not end_flag[0]:
                    ...
                    try:
                        ...
                        prev_vals_1 = env.mpc(orient_flag_1, prev_vals_1, env.car_1_state, env.nearest_idx_c1, obs_state_1, sys_params, max_L, L, Ts)
            ...
            if not end_flag[1]:
                try:
                    ...
                    obs_state_2 = env.const_vel_model(env.car_1_state, Ts, env.P + 1)
                    prev_vals_2 = env.mpc(orient_flag_2, prev_vals_2, env.car_2_state, env.nearest_idx_c2, obs_state_2, sys_params, max_L, L, Ts)
            ...
            #updating the states of other car GTC
        ...
        # Save all Dataclass object lists to a file
            env.save_log('../../Data/states_1_gtc_diff_speed_ahead.pickle', env.states_1)
            env.save_log('../../Data/controls_1_gtc_diff_speed_ahead.pickle', env.controls_1)
            env.save_log('../../Data/states_2_gtc_diff_speed_ahead.pickle', env.states_2)
            env.save_log('../../Data/controls_2_gtc_diff_speed_ahead.pickle', env.controls_2)
