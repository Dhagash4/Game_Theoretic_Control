...
# Game Theoretic Control based on Iterative Best Response for Two Cars
...
def main():
    try:
        ...
        weights1['w_steer']     = env.w_matrix(2, 0)[:-1, :-1]
        ...
        weights1['w_roc']       = 5
        weights1['w_c']         = env.w_matrix(init_value=2, step_size=0)[:-1, :-1]

        env.set_mpc_params(P = 25)
        weights2 = {}
        weights2['w_gas']       = env.w_matrix(1, 0)[:-1, :-1]
        weights2['w_steer']     = env.w_matrix(2, 0)[:-1, :-1]
