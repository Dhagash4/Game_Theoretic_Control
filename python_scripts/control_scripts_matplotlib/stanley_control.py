...
class Robot:
    ...
    def apply_control(self, a: float, steer_rate: float, dt: float) -> NoReturn:
        """
        Apply Control command to the robot
        
        Arguments
        ---------
        - a: acceleration value [m/s^2]
        - steer_rate: steering rate [rad/sec]
...
def longitudinal_controller(v: float, v_des: float, prev_err: float, cumulative_error: float, tuning_param: list, dt: float) -> Tuple[float, float, float]:
    """
    ...
    - tuning_param: [kp, ki, kd] PID controller tuning parameters
...
def calculate_target_index(robot: Robot, xs_des: list, ys_des: list) -> Tuple[int, float]:
    # Front axle position
...
def steer_controller(delta_des: float, delta: float, cumulative_error: float, prev_err: float, tuning_param: list, dt: float) -> Tuple[float, float, float]:
    """
    Compute control signal (steering rate) for lateral position control

    Arguments
    ---------
    - l_d: distance from robot to lookahead point on desired trajectory
    - alpha: angle between robot heading and the direction from robot to lookahead point on desired trajectory [rad]
    - delta: robot's current steering angle [rad]
    - prev_error: steering angle error from previous control loop iteration for derivative controller
    - cumulative_error: accumulated error over all previous control loop iterations for integral controller
    - tuning_param: [kp, ki, kd] PID controller tuning parameters
    - dt: Controller time step
    ...
    Returns
    -------
    - delta_dot: steering rate control signal [rad/sec]
    - curr_err: steering angle error from the current control loop
    - cumulative_error: accumulated error upto current control loop
    """ 
    # Extract PID tuning parameters
    [kp, ki, kd] = tuning_param
...
if __name__=="__main__":
    ...
    # np.savetxt('../../Data/Stanley_Controller/stanley_matplotlib.txt', data[0])
