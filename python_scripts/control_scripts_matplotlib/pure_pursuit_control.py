...
def calculate_lookahead_index(robot: Robot, xs_des: list, ys_des: list(), Ld: float) -> Tuple[float, int, bool]:
...
def lateral_controller(l_d: float, alpha: float, delta: float, cumulative_error: float, prev_err: float, tuning_param: list, dt: float) -> Tuple[float, float, float]:
    """
