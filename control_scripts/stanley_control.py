        ...
        while 1:
            ...
            if (idx == waypoints.shape[0] - 1) and idx != prev_idx:
                ...
                if laps_completed == laps_required:
                    while v_lon != 0.0:
                        self.vehicle.apply_control(carla.VehicleControl(throttle = 0, steer = 0, reverse = False, brake = 0.6))
                        v_lon = self.vehicle.get_velocity().x * np.cos(np.radians(yaw)) + self.vehicle.get_velocity().y * np.sin(np.radians(yaw))
                        v_lat = self.vehicle.get_velocity().x * np.sin(np.radians(yaw)) - self.vehicle.get_velocity().y * np.cos(np.radians(yaw))
                    break
            ...
            self.vehicle.apply_control(carla.VehicleControl(
                throttle = 1.0, steer = 0.0, reverse = False, brake = 0.0))
...
def main():
    try:
        ...
        # Load waypoints
        waypoints = env.read_file("2D_waypoints_modified.txt")
        ...
        # Save all Dataclass object lists to a file
            env.save_log('states_throttle_%f.pickle'%(1.0), env.states)
            # env.save_log('controls_e(%f)_v(%f)_p(%f)_i(%f)_d(%f)_n(%f).pickle'%(env.ke, env.kv, env.kp, env.ki, env.kd, num_of_laps), env.controls)
            # env.save_log('errors_e(%f)_v(%f)_p(%f)_i(%f)_d(%f)_n(%f).pickle'%(env.ke, env.kv, env.kp, env.ki, env.kd, num_of_laps), env.errors)
